...
void storeCodes(struct MinHeapNode* root, int arr[], int top) {
    ...
    if (isLeaf(root)) {
        ...
        // Added this print statement to show the mapped character values as they generate
...
void decodeText(struct MinHeapNode* root, const char* encodedStr) {
    if (root == NULL || encodedStr == NULL || encodedStr[0] == '\0') {
        return;
    }
...
int main() {
    ...
    if (size == 0) {
        printf("No characters entered to encode.\n");
        return 0;
    }

    struct MinHeapNode* root = buildHuffmanTree(chars, charFreq, size);

    // This section now prints the dictionary header first
    printf("\n--- Generated Character Dictionary ---\n");
    int arr[MAX_TREE_HT];
